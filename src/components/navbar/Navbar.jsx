...
function Navbar() {
    ...
    const userRole = decodeUserToken()?.roles[0]; // Assuming roles is an array and taking the first role
    ...
    return (
        <Disclosure as="nav" className="bg-gray-900 sticky top-0 z-50 ml-64 shadow-colorful-b">
                            ...
                            {isLoggedIn && <div className="hidden sm:ml-6 sm:block">
                                <div className="flex space-x-4">
                                    {navigation.map((item) => (
                                        <a
                                            key={item.name}
                                            href={item.name === 'Dashboard' ? (userRole === 'BOSS' ? '/bossDashboard' : '/store') : item.href}
            ...
            {navigation.map((item) => (
                ...
                key={item.name}
                    as="a"
                    href={item.name === 'Dashboard' ? (userRole === 'BOSS' ? '/bossDashboard' : '/store-dashboard') : item.href}
                    className={classNames(
                        item.current ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white',
