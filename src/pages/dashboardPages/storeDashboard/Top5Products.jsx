  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError(
          `Error fetching top 5 most profitable products: ${error.message}`
        );
        setLoading(false);
      }
    };
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg w-full h-full flex flex-col justify-between">
      <div>
        <h2 className="text-2xl font-bold mb-6 text-center">
          {top
            ? translate(getLanguage(), "top5MostProfitableProducts")
            : translate(getLanguage(), "bottom5LeastProfitableProducts")}
        </h2>
